# SOURCE: https://github.com/tokenbender/mHC-manifold-constrained-hyper-connections/blob/main/hyper_connections/hyper_connections_mhc.py
# PAPER:  "mHC: Manifold-Constrained Hyper-Connections"
#         DeepSeek — arXiv:2512.24880 (December 2025)
# ALSO:   "mHC-lite: You Don't Need 20 Sinkhorn-Knopp Iterations" — arXiv:2601.05732
#
# RELEVANCE TO MoLE:
#   - sinkhorn_log: log-space Sinkhorn-Knopp, projects H_res onto doubly stochastic matrix.
#     Note tau=0.05 temperature; num_iters=10 default (paper says 20, lite paper says fewer suffices).
#   - H_res_logits init: -8 everywhere except 0 on diagonal → starts near identity matrix.
#   - H_pre: softmax-normalized, also initialized to near-identity (diagonal=0, off-diag=-8).
#   - H_post: softmax-normalized.
#   - mhc_residual_identity_mix: optional alpha-blending between pure Sinkhorn and identity,
#     useful if you want even stronger identity bias early in training.
#   - width_connection: applies H_res (residual path) and H_pre (pre-sublayer path).
#   - depth_connection: applies H_post^T to add branch output back into streams.
#   - The stream dimension is folded into the batch dimension for sublayer calls
#     (standard trick: expand then reshape back).
...
from torch.nn import tree_flatten, tree_unflatten
