"""
H-Net official BoundaryRouter (RoutingModule) and DeChunkLayer source code.

Sourced verbatim from:
  https://github.com/goombalab/hnet/blob/main/hnet/modules/dc.py
  Commit: main branch as of 2026-03-17
  License: MIT

This file contains the complete dc.py from goombalab/hnet with annotations
added as block comments. All code within the class bodies is verbatim.

Key facts extracted for Zone E / BoundaryRouter verification:
  - W_q and W_k are initialized to identity (torch.eye(d_model))
  - _no_reinit = True prevents downstream re-initialization
  - p_1 is forced to 1.0 (PAD_PROB = 1.0) for sequence start
  - boundary_mask = (argmax(boundary_prob) == 1), i.e., threshold at 0.5
  - EMA in DeChunkLayer uses Mamba-2 scan kernel, not a naive loop
  - EMA step mode: result = p * h + (1-p) * last_value
  - plug_back uses cumsum(boundary_mask) - 1 for index gathering
"""
...
class RoutingModule(nn.Module):
    # NOTE: W_q and W_k are initialized to identity matrix.
    # This means at init, the routing module computes raw cosine similarity
    # between adjacent encoder hidden states with no learned projection yet.
    ...
    def forward(self, hidden_states, cu_seqlens=None, mask=None, inference_params=None):
        ...
        # NOTE: q is from tokens [0..L-2], k is from tokens [1..L-1].
        # So cos_sim[t] = cosine(q_{t}, k_{t+1}) for t in 0..L-2.
        # After padding, boundary_prob[t] measures dissimilarity between token t-1 and t.
        ...
        # boundary_prob shape: (..., 2) where [..., 0] = P(no boundary), [..., 1] = P(boundary)
...
class DeChunkLayer(nn.Module):
    # NOTE: EMA implemented via Mamba-2 scan kernel for efficiency.
    # The EMA formula is: z̄_t = P_t * ẑ_t + (1 - P_t) * z̄_{t-1}
    # where P_t = boundary_prob[..., -1] (the probability of this token being a boundary).
    # plug_back (Eq. 8): z̃_t = z̄_{cumsum(b)_t - 1} via torch.gather
    ...
    def forward(
        ...
        if cu_seqlens is not None:
            ...
            # Plug-back (Eq. 8): z̃_t = z̄_{cumsum(b)_t - 1}
            # Each position t gets the EMA output of the most recent boundary chunk.
    ...
    def step(self, hidden_states, boundary_mask, boundary_prob, inference_params):
        ...
        # EMA step: z̄_t = p * ẑ_t + (1-p) * z̄_{t-1}
