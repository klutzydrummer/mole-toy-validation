"""
Phase 2 Training Script — Outer Encoder Study.

Usage:
  python phase2/train.py --config outer_crl          # ALWAYS run first (cosine rule, no learned params)
  python phase2/train.py --config outer_crl_learned
  python phase2/train.py --config outer_transformer
  python phase2/train.py --config outer_diff_attn
  python phase2/train.py --config outer_mla
  python phase2/train.py --config outer_strided       # lower bound (identity encoder, fixed stride)
  python phase2/train.py --config outer_crl --resume

Loss (H-Net Eq. 10):
  loss = loss_ntp + alpha * loss_ratio
  loss_ntp   = cross_entropy(logits, targets)
  loss_ratio = (N / (N-1)) * ((N-1) * F * G + (1-F) * (1-G))
    F = (boundary_probs >= 0.5).float().mean()   hard fraction selected
    G = boundary_probs.mean()                     average soft probability
    N = 1 / target_rate                           target compression ratio (default: 4)
  alpha = 0.03 (H-Net default)

  outer_crl and outer_strided: alpha = 0 (no learned router to regularize).

Logs to checkpoints/<config>_seed<seed>.jsonl (same format as Phase 1 + hdc fields).
"""
...
# Configs with no learned router — compression ratio is fixed/cosine, alpha = 0
_NO_ROUTER_CONFIGS = {"outer_crl", "outer_strided"}
...
def boundary_entropy(boundary_probs: torch.Tensor) -> float:
    """Binary entropy of the boundary probability distribution.
    H(p) = -(p*log(p) + (1-p)*log(1-p)) averaged over all positions.
    High entropy = uncertain/uniform (bad). Low entropy = confident and selective (good).
    Clamp at 1e-6 not 1e-8: float32 ULP at 1.0 is ~1.19e-7 so 1e-8 doesn't prevent NaN."""
    p = boundary_probs.detach().float().clamp(1e-6, 1.0 - 1e-6)
    ent = -(p * p.log() + (1 - p) * (1 - p).log()).mean().item()
    return ent


def ratio_loss(boundary_probs: torch.Tensor, target_rate: float) -> torch.Tensor:
    """H-Net Eq. 10 ratio loss.

    ℒ_ratio = (N / (N-1)) * ((N-1) * F * G + (1-F) * (1-G))
      F = hard fraction selected (boundary_probs >= 0.5)
      G = average soft probability
      N = 1 / target_rate  (e.g. 4.0 for target_rate=0.25)

    Minimum = 1.0 when F = G = 1/N (both hard and soft rates match the target).
    """
    N = 1.0 / target_rate
    F = (boundary_probs >= 0.5).float().mean()
...
@torch.no_grad()
def evaluate_per_position(model, val_loader, device, max_batches=20, amp_dtype=torch.bfloat16):
    """
    Compute boundary BPC vs. mid-chunk BPC separately.
    Boundary positions: where boundary_probs >= 0.5.
...
def train(
    ...
    model = OuterModel(
        ...
        n_layers_outer=n_layers_outer, target_rate=target_rate,
    ).to(device)

    n_params = ParamCounter.count(model)

    # alpha = 0 for configs without a learned router
    effective_alpha = 0.0 if config in _NO_ROUTER_CONFIGS else alpha
    ...
    # gradient explosion that can occur when routing activates after the cold-start phase.
    _router = _unwrap(model).router
    ...
    _main_nodecay = [p for p in model.parameters()
                     if p.requires_grad and id(p) not in _router_ids and p.ndim < 2]
    _router_params = [p for p in model.parameters()
                      if p.requires_grad and id(p) in _router_ids]
    _param_groups = [
    ...
    if lit is not None:
        lit.log_hyperparams({
            ...
            "max_lr": max_lr, "alpha": effective_alpha,
            "target_rate": target_rate, "n_params": n_params,
        })
    ...
    print(f"target_rate: {target_rate:.2f} | alpha: {effective_alpha}")
    ...
    for step in range(start_step, total_steps):
        ...
        if (step + 1) % eval_interval == 0:
            ...
            print(f"  >>> val compression ratio: {val_ratio:.3f}"
                  f"  (target {_unwrap(model).target_rate:.2f})")
    ...
    summary = {
        ...
        "target_rate": target_rate, "alpha": effective_alpha,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Phase 2 Outer Encoder Training")
    parser.add_argument("--config",         type=str,   default="outer_crl",
                        choices=list(OuterModel.CONFIGS))
    ...
    parser.add_argument("--router_lr",      type=float, default=1e-4,
                        help="LR for router W_q/W_k. Lower than main to prevent cold-start explosion.")
    parser.add_argument("--alpha",          type=float, default=0.03,
                        help="H-Net ratio loss coefficient (Eq. 10). 0 for rulebased/stride.")
    parser.add_argument("--n_layers_outer", type=int,   default=4,
                        help="Depth of transformer-variant encoders.")
    parser.add_argument("--target_rate",    type=float, default=0.25,
                        help="Soft compression target (fraction of tokens selected as boundaries).")
